...
// Public marketplace feed — everyone (even logged out) sees every open lot here.
app.get('/api/lots', async (q, s) => s.json(await Lot.find({ deletedAt: null }).sort({ createdAt: -1 })));
// A user's own crop lots for their dashboard — farmers/FPOs see only what they listed;
// admins still see everything. Must stay above any '/api/lots/:id' route.
app.get('/api/lots/mine', auth, async (q, s) => {
  let scope = q.user.role === 'admin' ? { deletedAt: null } : { deletedAt: null, ownerId: q.user.id };
  s.json(await Lot.find(scope).sort({ createdAt: -1 }))
});
app.post('/api/lots', auth, allow('farmer', 'fpo'), async (q, s) => s.status(201).json(await Lot.create({ ...q.body, ownerId: q.user.id })));
app.patch('/api/lots/:id', auth, async (q, s) => {
  let lot = await Lot.findOne({ _id: q.params.id, deletedAt: null });
  if (!lot) return s.status(404).json({ error: 'Lot not found' });
  if (q.user.role !== 'admin' && String(lot.ownerId) !== q.user.id)
    return s.status(403).json({ error: 'Only the owner or an admin can update this lot' });
  s.json(await changed(Lot, q.params.id, q.body))
});
app.delete('/api/lots/:id', auth, async (q, s) => {
  let lot = await Lot.findOne({ _id: q.params.id, deletedAt: null });
  if (!lot) return s.status(404).json({ error: 'Lot not found' });
  if (q.user.role !== 'admin' && String(lot.ownerId) !== q.user.id)
    return s.status(403).json({ error: 'Only the owner or an admin can delete this lot' });
  await changed(Lot, q.params.id, { deletedAt: new Date() });
  s.json({ message: 'Removed' })
});
